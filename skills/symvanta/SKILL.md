...
- A cross-repo `relate` row may also carry `package`: the SCIP package name that
  joined the edge across repositories. `find_node` returns `scip_package_name` /
...
- `index_health` also reports `versionDrift` (a dependency pinned at one version
  ...
  an empty repo, `remote_head_unresolvable` = the remote could not be reached
  to resolve its head (network, credential, or timeout), so the branches were
  never read, `awaiting_upload` = an upload repo with no zip
  yet, `scip_runner_failed` = the precise indexer failed, so the index was
  withheld rather than published degraded and the job is retrying,
